...
def drawStickFigure(win, position, size):
    posx = position.getX()
    posy = position.getY()
    head = Circle(Point(posx, (posy-size)), (size/2))
    head.draw(win)
    body = Line(Point(posx, (posy-size/2)), Point(posx, (posy+size)))
    body.draw(win)
    arm1 = Line(Point(posx,posy), Point((posx-size/2), (posy+size/2)))
    arm1.draw(win)
    arm2 = Line(Point(posx,posy), Point((posx+size/2), (posy+size/2)))
    arm2.draw(win)
    leg1 = Line(Point(posx, (posy+size)), Point((posx-size/2), (posy+size*2)))
    leg1.draw(win)
    leg2 = Line(Point(posx, (posy+size)), Point((posx+size/2), (posy+size*2)))
    leg2.draw(win)

def drawStickFigureFamily():
    win = GraphWin("Stick Figure Family", 500, 400)
    drawStickFigure(win, Point(50,200), 50)
    drawStickFigure(win, Point(100,200), 30)
    drawStickFigure(win, Point(125,200), 10)
    drawStickFigure(win, Point(150,200), 20)
    drawStickFigure(win, Point(200,200), 60)
    win.getMouse()
#NOTE: FIX Y POS
